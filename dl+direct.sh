...
if [ ${DO_CT} -gt 0 ] ; then
	...
	VELOCITY_VOLUME=${DST}/T1w_ForwardVelocityField.nii.gz
	python ${SCRIPT_DIR}/surface.py "${VELOCITY_VOLUME}" "${DST}/seg.nii.gz" "${DST}/softmax_seg.nii.gz" "${SUBJECT_ID}"
fi
...
if [ ${KEEP_INTERMEDIATE} -eq 0 ] ; then
	rm -f ${DST}/{boundary*.nii.gz,?mprob*.nii.gz,seg.nii.gz,seg_*.nii.gz,softmax_seg.nii.gz,T1w_norm_noskull_*.nii.gz,T1w_thickmap.nii.gz,T1w_ForwardVelocityField.nii.gz,T1w_InverseVelocityField.nii.gz}
fi
